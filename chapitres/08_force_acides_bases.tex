...
Couples de l'eau : $H_30^+/H_2O$ et $H_2O/HO^-$\\
...
\begin{enumerate}
	...
	\item Calculer $\rm[H_30^+]_{eq}$ et $\rm[HO^-]_{eq}$. Comparer ces deux valeurs.
\end{enumerate}

\trou{$\rm[H_30^+]_{eq}=10^{-3,5}=\SI{3,2e-4}{\mole\per\liter}$\\
...
Ici $\rm x_f$ est différent de $\rm x_{max}$, ce qui signifie que la réaction n'est pas totale. La réaction atteint son état finale (plus de consommation de réactif) alors que les deux réactifs sont encore présents. On dit que l'équilibre chimique est atteint. L'avancement à l'état final est donc inférieur à $x_{max}$ qui correspond à la consommation du réactif limitant.
...
C'est le cas de l'acide éthanoique :
...
\hline
\end{tabular}\\
La réaction est totale donc on obtient $\rm x_{max}=\trou{CV = n_0 = n(H_30^+)_{eq}}$.\\
...
\subsection{Tau d'avancement}

\blocrouge{Définition}{ Pour indiquer l'aspect totale ou limitée d'une réaction on calcule le tau d'avancement $$\tau=\dfrac{x_f}{x_{max}}$$}

Si $\tau<1$ alors \trou{la réaction n'est pas totale}. Si $\tau=1$ alors \trou{la réaction est totale.}

Plus le tau d'avancement de la réaction d'un acide avec l'eau est proche de 1 plus cet acide se comporte comme un acide fort. Même raisonnement pour une base.
...
Exemple du diméthylamine de pKa égal à 10,8
