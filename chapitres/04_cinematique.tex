...
\trou{Nous sommes immobiles par rapport au sol et en mouvement par rapport au Soleil. Il est donc indispensable de définir un solide de référence: un référentiel.}
...
\begin{enumerate}
	...
	\item Un ballon de basket lors d'un lancer franc.
...
Le vecteur position est le vecteur qui relie l'origine du repère et le point mobile M. Il se note: $\overrightarrow{OM(t)}$ ses coordonnées sont les mêmes que celles du point M. 
...
\paragraph*{Remarques} L'unité de chaque coordonnée est le mètre (m). \\A l'instant initial t=0s, l'abscisse x se notera $x(t=0)$ ou $x(0)$.
...
Pour calculer, les coordonnées de $\vec{v}$ il faudra dériver par rapport au temps les fonctions $x(t)$, $y(t)$ et $z(t)$. 
...
\\On peut cependant écrire: $$\vec{v}=v_x(t).\vec{i}+v_y(t).\vec{j}$$ il y a bien des vecteurs à gauche et à droite du signe $=$.
...
Cela signifie que les coordonnées de $\overrightarrow{a(t)}$ s'obtiennent en dérivant les coordonnées  $v_x(t)$ et $v_y(t)$ du vecteur vitesse $\overrightarrow{v(t)}$.
...
& Vitesse constante & $\vec{a(t)}=\trou{\vec{0}}$&\\
\hline
...
Dans un mouvement circulaire uniforme, la valeur de la vitesse est constante. 
...
\blocgris{MCU}{
Dans un MCU, les coordonnées du vecteur accélération s'écrivent simplement:
