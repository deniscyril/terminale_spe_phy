...
\blocrouge{A savoir}{
	\begin{itemize}
		...
		\item Un élément est défini par son numéro atomique Z (c'est le nombre \trou{de proton}) et son symbole chimique.	
...
La physique nucléaire étudie les propriétés des noyaux des atomes. On observe que certains noyaux peuvent modifier leur composition. Ainsi le nombre de proton et de neutron est modifié (selon certaines règles...). Ces modifications s'appellent des transformations nucléaires ou encore radioactivité.
...
Il existe deux radioactivité beta selon que la particule émise est chargée positivement ou négativement.
\begin{itemize}
...
$$
  $$^{14}_{7}N \longrightarrow ^{14}_{6}C + \trou{e^- }$$
    ...
    Les éléments qui ont le même abscisse sont \trou{isotopes}.
...
\begin{itemize}
	...
	\item  Faisons tendre $\Delta t$ vers 0 pour estimer plus \trou{précisement} $A(t)$
...
Le nombre de noyaux radioactifs et l'activité d'un échantillon décroissent exponentiellement. Le temps caractéristique de cette décroissance est appelée \trou{la demi-vie} (analogue au temps de demi-réaction en chimie)
...
\etiquette{Exercice} Représenter graphiquement l'allure de $N(t)$ et indiquer comment on déterminer $t_{1/2}$
