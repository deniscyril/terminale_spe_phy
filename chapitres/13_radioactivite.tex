...
\blocrouge{A savoir}{
	\begin{itemize}
		...
		\item La notation symbolique d'un noyau  X est $$ {}^A_Z\mathrm{X}$$
		\item Deux noyaux possédant le même numéro atomique mais un nombre de masse différent sont appelés \trou{isotopes}.
	\end{itemize}

}

\etiquette{Application} On considère 3 noyaux: $_{40}^{93}Zr$ ; $_{92}^{238}U$ ; $_{40}^{95}Zr$. 
\begin{enumerate}
...
La physique nucléaire étudie les propriétés des noyaux des atomes. On observe que certains noyaux peuvent modifier leur composition. Ainsi le nombre de protons et de neutrons est modifié. Ces modifications s'appellent des transformations nucléaires ou encore radioactivité.
...
\begin{itemize}
	...
	\item Compléter les transformations nucléaires
\end{itemize}
...
$$
  $$ ^{14}_{6}C\longrightarrow  ^{14}_{7}N+ \trou{e^- }$$
  
     \subsection{Diagramme (N,Z)}
     
    Sur un diagramme, on repère un noyau par son nombre de protons Z (en abscisse) et son nombre de neutrons N (en ordonnée).
...
\begin{itemize}
	...
	\item A l'instant t>0, on veut estimer le nombre de noyaux qui se désintègrent par unité de temps: $A(t)$.
...
$\lambda$ est appelée la constante radioactive, sa valeur dépend du noyau considéré. Son unité est \trou{l'inverse de la seconde ou $s^{-1}$}.
...
\etiquette{SAVOIR FAIRE} Représenter graphiquement l'allure de $N(t)$ et indiquer comment on détermine $t_{1/2}$

\newpage
\etiquette{Ordres de grandeur de la demi-vie}
\begin{table}[htp!]
\centering
\renewcommand{\arraystretch}{1.4}
\begin{tabular}{|l|c|c|c|}
\hline
\textbf{Noyau} & \textbf{Symbole} & \textbf{Type} & \textbf{Demi-vie $t_{1/2}$} \\
\hline
Polonium-214    & $^{214}_{84}$Po  & $\alpha$      & $1{,}6 \times 10^{-4}$ s \\
Radon-222       & $^{222}_{86}$Rn  & $\alpha$      & $3{,}8$ jours \\
Iode-131        & $^{131}_{53}$I   & $\beta^-$     & $8{,}0$ jours \\
Cobalt-60       & $^{60}_{27}$Co   & $\beta^-$     & $5{,}3$ ans \\
Tritium         & $^{3}_{1}$H      & $\beta^-$     & $12{,}3$ ans \\
Carbone-14      & $^{14}_{6}$C     & $\beta^-$     & $5\,730$ ans \\
Plutonium-239   & $^{239}_{94}$Pu  & $\alpha$      & $2{,}4 \times 10^{4}$ ans \\
Uranium-235     & $^{235}_{92}$U   & $\alpha$      & $7{,}0 \times 10^{8}$ ans \\
Uranium-238     & $^{238}_{92}$U   & $\alpha$      & $4{,}5 \times 10^{9}$ ans \\
\hline
\end{tabular}
%\caption{Ordres de grandeur de la demi-vie de quelques noyaux radioactifs.}
\label{tab:demivies}
\end{table}
